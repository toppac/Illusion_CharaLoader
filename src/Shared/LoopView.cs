...
namespace CharaLoader
{
    public class CharaItem
    {
        ...
        // Sex == 0 ? Male : Female
        public byte Sex { get; set; } = byte.MaxValue;
        public bool EvoVersion { get; set; }
        //public string FileFullPath { get; set; }
        public string CharaName { get; set; }
        public Texture2D CharaCoverTex2D { get; set; }
        ...
        private static ArrayPool<byte> _poolArr = ArrayPool<byte>.Create();
                ...
                using (var br = new BinaryReader(fs))
                {
                    ...
                    if (dataPos > PNG_MAX_LENGTH)
                    {
                        UnityEngine.Debug.Log(
                            $"Over 6MB: {dataPos} \r{Path.GetFileName(fileFullPath)}");
                        return false;
                    }
#endif
                    if (dataPos == 0) return false;
                    if (GetPngData(br, dataPos)) return true;
                }
            }
            return false;
        }
        ...
        {
            var result = false;
            if (dataLength > PNG_MAX_LENGTH) return result;
            var buffer = _poolArr.Rent(dataLength);

            try
            {
                if (br.BaseStream.Read(buffer, 0, dataLength) == dataLength
                    && CharaCoverTex2D.LoadImage(buffer))
                    IsPng = result = true;
            }
            finally
            {
                _poolArr.Return(buffer);
            }
            return result;
        }
     ...
     public class ItemComa<T> : VirtualItem<T> where T : class
    {
        ...
        public static readonly Color UnStart = new Color(0.556f, 0.556f, 0.556f);
    ...
    internal class CharaComa : ItemComa<string>
    {
        ...
        private Texture2D CoverTex2D
            = new Texture2D(2, 2, TextureFormat.RGB24, true);

        public static HashSet<string> RecycleItems { get; private set; }
            = new HashSet<string>();
        ...
        public override void Init(GameObject obj, int objIndex)
        {
            ...
            _charaData.CharaCoverTex2D = CoverTex2D;
        ...
        public void InitMakerCover(Transform coverTrans)
        {
            ...
            var faceEvn = panel.Find("FaceBtn")
                .gameObject.AddComponent<ClickListener>();
            faceEvn.LClick += LoadFace;
            faceEvn.RClick += LoadFaceBone;

            var clothEvn = panel.Find("ClothBtn").gameObject
        ...
        public override void Refresh()
        {
            ...
            if (Time.time - _timeMinmise < 0.3f)
            {
                SetVisible(true);
...
#else
            SetVisible(true);
            if (ReadFileInfo.LoadFile(ItemData, _charaData))
            {
                CharaEvn.LClick += ActiveCover;
                CharaEvn.LClick += ActiveStarButton;
        ...
        private void LoadBodyOverlay(PointerEventData e)
        {
            if (Loader.GMod == KKAPI.GameMode.Maker)
            {
                MakerTools.Instance?.LoadCustom(
                    ItemData, (int)LOpt.SkinOver);
                return;
            }
            var cf = new ChaFile();
            cf.LoadFile(ItemData, 0, true, true);
        ...
        private void LoadAcc()
        {
            if (Loader.GMod == KKAPI.GameMode.Maker)
            {
                MakerTools.Instance?.LoadReplaceParts(
                    ItemData, (int)LOpt.Accessory);
                return;
            }
        ...
        {
            MakerTools.Instance?.LoadCustom(
                ItemData, (int)LOpt.ChaParam);
        }

        private void LoadFaceKeepShader()
        {
            MakerTools.Instance?.LoadCustom(
                ItemData, (int)LOpt.FaceBone);
        }

        private void LoadBodyKeepShader()
        {
            MakerTools.Instance?.LoadCustom(
                ItemData, (int)LOpt.BodyBone);
        }
        ...
        private void LoadChara()
        {
            ...
            if (HSceneTools.HSceneIns)
            {
                Loader.StartCoroutine(
                    HSceneTools.ChangeCharacter(ItemData, _charaData.Sex));
                return;
            }
#endif
    ...
    public class LoopList<TItem, TData> where TItem : VirtualItem<TData>, new() where TData : class
    {
        ...
        public PanelOffset Border => _border;
        public int EndPos => _endPos;

        public Action OnScrollValueChanged { get; set; }
        ...
        public List<TData> DataList
        {
            get
            {
                return _dataList ?? (_dataList = new List<TData>());
            }

            set { _dataList = value; }
        }

        public void InitView(GameObject ItemObj, ScrollRect scrollComp, RectTransform ViewRectTrans, PanelOffset border, Action scrollValueChanged = null)
        {
            if (_isInit) return;
            ItemPrefab = ItemObj;
            ...
            {
                OnScrollValueChanged = scrollValueChanged;
            }
        ...
        private void SetContentHeight()
        {
            ...
            var dataCount = _endPos;

            var lineCount = dataCount % RowNum > 0
                ? dataCount / RowNum + 1 : dataCount / RowNum;
        ...
        public void ScrollValueChanged(Vector2 deltaPos)
        {
            ...
            _dragY = deltaPos.y;
#if DEBUG
            //UnityEngine.Debug.Log(deltaPos.y);
#endif
            while (true)
            {
                if (_isUp)
                {
                    ...
                    if (f1 > ItemHeight && TailDataId < _endPos - 1)
                    {
                        ...
                        SwapItemView(true);
#else
                        ReCharaLoaderStudio.Instance
                            .StartCoroutine(SwapItemView(true));
                        //SwapItemViewTryData(true);
#endif
...
#else
                    ReCharaLoaderStudio.Instance
                            .StartCoroutine(SwapItemView(false));
                    //SwapItemViewTryData(true);
#endif
                    _topLeftIndex -= RowNum;
                    continue;
        ...
        {
            _endPos = dataPos + readCount <= _dataCount
                ? readCount + dataPos : _dataCount;
            ...
            foreach (var iv in _itemsView)
            {
                ...
                {
                    if (iv.Value.TryGetData(DataList[dataPos]))
                    {
                        iv.Value.SetItemData(DataList[dataPos], dataPos);
                        ++dataPos;
                        break;
...
#if UNITY_TEST
        private void SwapItemViewTryData(bool next)
        {
            if (next)
            {
                for (var i = 0; i < RowNum; i++)
                {
                    var itemRect = _itemsRect.First.Value;
                    var vItem = _itemsView[itemRect.gameObject];

                    var nId = vItem.DataIndex + 1;
                    for (; nId < _endPos; nId++)
                    {
                        if (vItem.TryGetData(DataList[nId]))
                        {
                            break;
                        }
                    }
            ...
            for (var j = 0; j < RowNum; j++)
            {
                var itemRect = _itemsRect.Last.Value;
                var vItem = _itemsView[itemRect.gameObject];
                var pId = vItem.DataIndex - 1;
                for (; pId < _endPos; pId--)
                {
                    if (vItem.TryGetData(DataList[pId]))
                    {
                        break;
                    }
                }
    ...
    public struct PanelOffset
    {
        ...
        public static PanelOffset Zero =>
            new PanelOffset(0, 0, 0, 0, new Vector2(0, 0));
    }
}
